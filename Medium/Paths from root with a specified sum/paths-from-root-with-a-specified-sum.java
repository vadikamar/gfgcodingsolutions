...
class Solution
{
    public static void check(Node root,int s,int sum,ArrayList<ArrayList<Integer>> x,ArrayList<Integer> y)
    {
        if(root==null) return;
        s+=root.data;
        y.add(root.data);
        if(s==sum) x.add(new ArrayList<>(y));
        check(root.left,s,sum,x,y);
        check(root.right,s,sum,x,y);
        y.remove(y.size()-1);
    }
