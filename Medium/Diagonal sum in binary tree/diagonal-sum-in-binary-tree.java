...
class Tree {
    public static ArrayList <Integer> diagonalSum(Node root) 
    {
        ...
        while(!qt.isEmpty())
        {
            ...
            if(temp.containsKey(pt.level))
            {
                temp.put(pt.level,temp.get(pt.level)+ pt.node.data);
            }
            else
            {
                temp.put(pt.level,pt.node.data);
            }
            
            if(pt.node.left != null)
            {
                qt.offer(new Pair(pt.level+1,pt.node.left));
            }
            
            if(pt.node.right != null)
            {
                qt.offer(new Pair(pt.level,pt.node.right));
            }
